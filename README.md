***REMOVED*** 行途 XingTu · 个人作品集站点（代码即文档）
...
```
xingtu-site/
├── index.html    ***REMOVED*** 站点本体（零依赖单文件，内容数组化）
├── deploy.sh     ***REMOVED*** 一键部署脚本（wrangler）
├── README.md     ***REMOVED*** 本文件（源码 + 管理手册）
```

---

***REMOVED******REMOVED*** 🔑 管理入口（控制台直达）
...
***REMOVED******REMOVED*** 🧠 管理原理（白盒，看懂就能自诊）

***REMOVED******REMOVED******REMOVED*** 部署链路（三层）
```
...
***REMOVED******REMOVED******REMOVED*** 发布原理（不可变快照）
...
***REMOVED******REMOVED******REMOVED*** 7. 吊销/轮换凭证（安全）
| 凭证 | 管理入口 | 备注 |
|------|----------|------|
...
***REMOVED******REMOVED*** 🚀 快速开始（本地预览）

```bash
python3 -m http.server 8000   ***REMOVED*** 打开 http://localhost:8000
```

***REMOVED******REMOVED*** ⚠️ 避坑提醒（血泪经验）
